...
appActions.errorText = {
  ...
  E_NO_TIMEZONE_SELECTED : 'No timezone was selected'
};
...
appActions._afterLoginPage = function(user, cb) {
  ...
  var targetDevicesById = devices[defaultTargetId];
  var targetDevices = _.pluck(targetDevicesById, 'key');
...
appActions.storeUserTargets = function(targetId) {
  ...
  var devicesById = localStore.getItem('devices') || {};
  var targetDevices = this.app.state.targetDevices;
  var devicesToStore = _.map(targetDevices, function(deviceKey) {
    return {
      key: deviceKey
    };
  });
  devicesById[targetId] = devicesToStore;
...
appActions.upload = function(uploadIndex, options, cb) {
  ...
  // If no timezone is selected, the Upload button is disabled
  // So we're just double-checking here
  if (_.isEmpty(targetTimezone)) {
    var err = new Error(self.errorText.E_NO_TIMEZONE_SELECTED);
