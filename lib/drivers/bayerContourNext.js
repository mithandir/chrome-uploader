...
module.exports = function (config) {
  ...
  */
  ...
  function decodeMessage (message) {
    if (!(message.charCodeAt(0) === ASCII_CONTROL.STX &&
      message.charCodeAt(message.length-2) === ASCII_CONTROL.CR &&
      message.charCodeAt(message.length-1) === ASCII_CONTROL.LF ) &&
      message.charCodeAt(0) !== ASCII_CONTROL.EOT ) {
    ...
    var buf = new Uint8Array(struct.packString(message));
    var response = struct.unpack(buf, 0, 'bb'+frameLength+'Z2Z2Z', ['messageType', 'sequenceNumber', 'frame', 'checksum', 'CRLF']);
  ...
  */
  ...
  var bcnCommandResponse = function (commandpacket, callback) {
    hidDevice.send(commandpacket, function () {
      getASTMMessage(5000, 3, function(err) {
        if (err !== null || !hidDevice.hasAvailablePacket()) {
          return callback(err, null);
        } else {
          // We're using the hidDevice as a message buffer, not a USB packet buffer
          // to save on buffer copying. It could be refactored differently easily enough.
          var message = hidDevice.nextPacket();

          try {
            callback(null, decodeMessage(message.payload));
          }
          catch(err) {
            return callback(err, null);
          }
  ...
  var getASTMMessage = function (timeout, retries, callback) {
    ...
    }, 20); // The Contour timeout is 15 seconds, but 20 milliseconds should be enough to be kind to the CPU.
  };
  ...
  var prepBGData = function (progress, data) {
    ...
    if (data.bgmReadings.length > 0) {
      for (var i = 0; i < data.bgmReadings.length; ++i) {
        ...
        if(datum.glucose != null && datum.displayTime != null && datum.timezoneOffset != null && datum.conversionOffset != null && datum.displayUtc != null && datum.units != null && datum.nrec != null) {
